<div class="container-fluid mt-5">
    <span class="badge rounded-pill text-bg-info mb-3 fw-bold text fs-5" style="height: 2.3rem;">On Progress</span>
    <table class="table table-bordered table-striped">
        ...
        <tbody>
            <?php 
            // Update query untuk hanya menampilkan pekerjaan dengan status 'DONE'
            $query = "SELECT * FROM pekerjaan WHERE nipp = '$nipp' AND status = 'On Progress' ORDER BY 
              CASE 
    ...
    $deadline = htmlspecialchars($row['deadline']);
    if ($deadline && $status == 'On Progress') {
        $today = new DateTime();
    ...
    ?>
            ...
            <?php else: ?>
            <tr>
                <td colspan="9" class="text-center">Tidak ada pekerjaan dengan status 'On Progress' ditemukan untuk NIPP
                    ini.</td>
            </tr>
            <?php endif; ?>
        </tbody>
